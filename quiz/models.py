...
CATEGORY_OPTIONS = (
    ...
    ("exam", _("Экзамен")),
    ("practice", _("Практическое задание")),
)
...
class Quiz(models.Model):
    ...
    category = models.CharField(choices=CATEGORY_OPTIONS, max_length=50, blank=True)
    ...
    exam_paper = models.BooleanField(
        blank=False,
        default=False,
        verbose_name=_("Экзаменационный лист"),
        help_text=_(
    ...
    def save(self, force_insert=False, force_update=False, *args, **kwargs):
        ...
        if not (0 <= self.pass_mark <= 100):
            raise ValidationError(f"{self.pass_mark} находится вне диапазона (0-100)")
...
class Progress(models.Model):
    ...
    class Meta:
        ...
        verbose_name_plural = _("Записи прогресса пользователей")
    ...
    def update_score(self, question, score_to_add=0, possible_to_add=0):
        ...
        if any(
            ...
            return _("ошибка"), _("категория не существует или недопустимый счет")
...
class Sitting(models.Model):
    ...
    def result_message(self):
        if self.check_if_passed:
            ...
            return f"Вы провалили этот тест, дайте ему еще один шанс."
...
class Question(models.Model):
    ...
    figure = models.ImageField(
        upload_to="figure_pictures/", null=True,
        blank=True,
    ...
    explanation = models.TextField(
        max_length=2000,
        blank=True,
        help_text=_("Объяснение будет приведено после того, как будет дан ответ на вопрос."),
...
class MCQuestion(Question):
    choice_order = models.CharField(
        ...
        help_text=_(
            "Порядок, в котором пользователю отображаются параметры выбора с несколькими вариантами выбора"
        ),
    ...
    def order_choices(self, queryset):
        if self.choice_order == "Контент":
            return queryset.order_by("choice")
        if self.choice_order == "Случайно":
            return queryset.order_by("?")
        if self.choice_order == "Нет":
            return queryset.order_by()
        return queryset
    ...
    class Meta:
        ...
        verbose_name_plural = _("Вопросы с множественным выбором")
...
class Choice(models.Model):
    ...
    choice = models.CharField(
        max_length=1000,
        blank=False,
        help_text=_("Введите выбранный текст, который вы хотите отобразить"),
