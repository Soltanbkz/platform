...
from .models import User, Student, Parent, RELATION_SHIP, GENDERS
...
class StaffAddForm(UserCreationForm):
    ...
    def save(self, commit=True):
        ...
        generated_username = f"{settings.LECTURER_ID_PREFIX}-{registration_date}-{total_lecturers_count}"

        user.username = generated_username

        if commit:
            user.save()
            send_new_lecturer_email.delay(user.pk, self.cleaned_data.get("password1"))
...
class StudentAddForm(UserCreationForm):
    ...
    def save(self, commit=True):
        ...
        generated_username = f"{settings.STUDENT_ID_PREFIX}-{registration_date}-{total_students_count}"
        ...
        if commit:
            ...
            send_new_student_email.delay(user.pk, self.cleaned_data.get("password1"))
...
class ParentAddForm(UserCreationForm):
    ...
    def save(self, commit=True):
        ...
        # Generate a username based on first and last name and registration date
        registration_date = datetime.now().strftime("%Y")
        total_parents_count = Parent.objects.count()
        generated_username = f"{settings.PARENT_ID_PREFIX}-{registration_date}-{total_parents_count}"
...
# Определение класса UserAdmin, если он используется в forms.py
